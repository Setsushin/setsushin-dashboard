// user-menu.jsx — top-right avatar + email + logout popover.
// Self-contained chrome that owns its own popover state but reads
// identity (`me`) as a prop from App so /api/me is fetched once.
  ...
  return (
    <div className="user-menu" ref={wrapRef}>
      <button className="avatar-btn" aria-label="Account"
        ...
        {initial}
        {avatarUrl && (
          <img key={avatarUrl} className="avatar-img" src={avatarUrl} alt=""
               onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        )}
      </button>
      {open && (
        <div className="user-pop" role="menu">
          <div className="user-pop-email" title={email || ''}>{email || '—'}</div>
          {me?.local ? (
            <div className="user-pop-note">local dev · no logout</div>
          ) : (
            <a className="user-pop-logout" role="menuitem"
               href={`/cdn-cgi/access/logout?returnTo=${encodeURIComponent(window.location.origin + '/')}`}
               onClick={(e) => { if (!window.confirm('Log out of the dashboard?')) e.preventDefault(); }}>
              Log out
            </a>
          )}
        </div>
