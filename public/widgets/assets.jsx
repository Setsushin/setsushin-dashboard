// widgets/assets.jsx — Portfolio widget backed by D1.
//
// Reads /api/assets (D1) + /api/fx (rates).
// All edits are kept in 万JPY (the canonical unit); the header currency
...
// Warm taupe for JPY (was a cool dusty blue, the only out-of-family
// color in the chart). Sits between sage L2 and honey L3 on the warm
// spectrum but stays distinct from both.
const CCY_META = {
...
const EXPOSURE_LABEL = { jpy: 'JPY', usd: 'USD', [EXPOSURE_MIXED]: 'mix' };
const EXPOSURE_LABEL_LONG = { jpy: 'JPY', usd: 'USD', [EXPOSURE_MIXED]: 'mix 50/50' };

// jpy_man (1 万JPY) → display unit per currency. USD: man × USD/JPY × 10
// (since 1 万 = 10 K). /api/fx always returns rates (with stale fallback),
// so we trust fx?.USD / fx?.CNY without a local fallback number.
const CCY_DISP = {
...
function fmtAmount(jpyMan, ccy, fx) {
...
function arcPath(cx, cy, rIn, rOut, startA, endA) {
  const x1 = cx + rOut * Math.cos(startA), y1 = cy + rOut * Math.sin(startA);
  const x2 = cx + rOut * Math.cos(endA),   y2 = cy + rOut * Math.sin(endA);
  const x3 = cx + rIn  * Math.cos(endA),   y3 = cy + rIn  * Math.sin(endA);
  const x4 = cx + rIn  * Math.cos(startA), y4 = cy + rIn  * Math.sin(startA);
  const large = endA - startA > Math.PI ? 1 : 0;
  return `M ${x1} ${y1} A ${rOut} ${rOut} 0 ${large} 1 ${x2} ${y2} L ${x3} ${y3} A ${rIn} ${rIn} 0 ${large} 0 ${x4} ${y4} Z`;
}

function tint(hex, t = 0.35) {
...
function byCurrency(items) {
  const expanded = items.flatMap(it => it.exposure === EXPOSURE_MIXED
    ? [{ ...it, _ccy: 'jpy', jpy_man: it.jpy_man / 2 },
       { ...it, _ccy: 'usd', jpy_man: it.jpy_man / 2 }]
    : [{ ...it, _ccy: it.exposure }]);
  return ['jpy', 'usd'].map(c => {
...
// LLM-friendly snapshot. Always emits the canonical unit (万JPY) plus the
// current FX rates so a downstream model can re-convert without guessing.
function buildMarkdown(items, fx) {
  const today = new Date().toISOString().slice(0, 10);
  const grand = items.reduce((s, i) => s + i.jpy_man, 0);
  const fmt = (n, d = 1) => n.toLocaleString('en-US', {
  ...
  for (const L of LAYER_ORDER) {
    ...
    for (const it of layerItems) {
      const meta = [
        EXPOSURE_LABEL_LONG[it.exposure] ?? it.exposure,
        it.account,
      ].filter(Boolean).join(' · ');
...
function Sunburst({ groups, ccy, fx, total }) {
  const SIZE = 280, CX = SIZE / 2, CY = SIZE / 2;
  const R_IN1 = 56, R_OUT1 = 84;
  const R_IN2 = 86, R_OUT2 = 124;
  const TWO_PI = Math.PI * 2;
  const offset = -Math.PI / 2;

  const [hover, setHover] = useState(null);

  // Slices are derived from groups only; hover changes opacity, not layout.
  const { inner, outer } = React.useMemo(() => {
    const grand = groups.reduce((s, g) => s + g.total, 0) || 1;
    const inner = [], outer = [];
    let acc = 0;
    for (const g of groups) {
      const sweep = (g.total / grand) * TWO_PI;
      const a0 = offset + acc, a1 = offset + acc + sweep;
      inner.push({
        key: `i-${g.key}`,
      ...
      for (const c of g.children) {
        ...
        const ca0 = a0 + inner_acc, ca1 = a0 + inner_acc + cs;
        outer.push({
          key: `o-${g.key}-${c.id}-${c._ccy || ''}`,
          d: arcPath(CX, CY, R_IN2, R_OUT2, ca0, ca1),
          color: tint(c.color, 0.25),
          tip: {
            kind: 'item', label: c.name, amount: c.jpy_man,
            pct: (c.jpy_man / grand) * 100,
            layer: c.layer,
            exposure: c.exposure, account: c.account,
          },
        });
        inner_acc += cs;
      }
  ...
  const onMove = (tip) => (e) => {
    const rect = e.currentTarget.closest('.sunburst-wrap').getBoundingClientRect();
  ...
  const slicePath = (s) => (
    <path key={s.key} d={s.d} fill={s.color}
          stroke="var(--bg-content)" strokeWidth="1"
...
function SunburstTip({ hover, ccy, fx }) {
  return (
...
function AssetForm({ initial, layer: defaultLayer, onSave, onCancel }) {
  ...
  const [exposure, setExposure] = useState(initial?.exposure ?? 'jpy');
  ...
  return (
    <div className="assets-row assets-row-form" onKeyDown={onKey}>
      <input ref={nameRef} className="af-input af-name"
             placeholder="Name" value={name} onChange={e => setName(e.target.value)} />
      <input className="af-input af-amount" type="number" step="0.1"
             placeholder="万JPY" value={jpyMan} onChange={e => setJpyMan(e.target.value)} />
      ...
      <select className="af-input af-exp" value={exposure} onChange={e => setExposure(e.target.value)}>
        {EXPOSURE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
      </select>
      <input className="af-input af-acct"
             placeholder="Account" value={account} onChange={e => setAccount(e.target.value)} />
      <div className="af-actions">
        <button className="af-btn af-save"   onClick={submit} title="Save (Enter)">✓</button>
        <button className="af-btn af-cancel" onClick={onCancel} title="Cancel (Esc)">✕</button>
      </div>
    </div>
  );
}

function AssetRow({ item, grand, ccy, fx, onEditStart, onDelete }) {
  return (
    <div className="assets-row">
      ...
      <span className={`assets-row-tag ex-${item.exposure}`}>
        {EXPOSURE_LABEL[item.exposure] ?? item.exposure}
      </span>
      <span className="assets-row-account muted">{item.account}</span>
      <div className="assets-row-actions">
        <button className="assets-row-act" onClick={onEditStart} title="Edit">✎</button>
        <button className="assets-row-act assets-row-del" onClick={onDelete} title="Delete">×</button>
      </div>
    </div>
  );
}

function AssetsWidget({ config }) {
  const [assets, setAssets] = useState(null);
  const [ccy, setCcy] = useState(config?.default_currency || 'JPY');
  const [view, setView] = useState('layer');
  const [collapsed, setCollapsed] = useState({});
  const [editingId, setEditingId] = useState(null);
  const [addingLayer, setAddingLayer] = useState(null);
  const [copyAck, setCopyAck] = useState(false);

  const fxState = useFetch('/api/fx?base=JPY&symbols=USD,CNY', { ttl: 60 * 60_000 });
  ...
  // Optimistic mutate: apply locally, then call server. POST must reload to
  // pick up the server-assigned id; PATCH/DELETE only reload on failure.
  ...
  const create = (form) => mutate(
    'POST',
    () => { setAddingLayer(null); setAssets(a => [...(a || []), { ...form, id: -Date.now() }]); },
    () => fetch('/api/assets', {
      method: 'POST',
  ...
  const update = (id, patch) => mutate(
    'PATCH',
    () => { setEditingId(null); setAssets(a => (a || []).map(it => it.id === id ? { ...it, ...patch } : it)); },
    () => fetch(`/api/assets/${id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  const title = (
    <span className="assets-title">
      <span>Portfolio</span>
      <button className="assets-md-chip"
              onClick={onCopyMd}
  ...
  const fxStaleHint = fxState.data?.stale
    ? mockHint({ error: true, reason: `fx ${fxState.data.source}` })
    : null;

  if (assets === null) {
    return <Panel title={title} action={action} className="panel-wide">
      ...
      <div className="assets-split">
        <div className="assets-list">
          {LAYER_ORDER.map(layer => {
            ...
            return (
              <div key={layer} className="assets-layer">
                <div className="assets-layer-head"
                     onClick={() => setCollapsed(c => ({ ...c, [layer]: !c[layer] }))}>
                  <span className="assets-layer-caret">{isCollapsed ? '▶' : '▼'}</span>
                  <span className="assets-layer-color" style={{background: LAYER_META[layer].color}}></span>
                  <span className="assets-layer-name">{layer} {LAYER_META[layer].label}</span>
                ...
                {!isCollapsed && (
                  <div className="assets-layer-body">
                    {layerItems.map(it => editingId === it.id
                      ? <AssetForm key={it.id} layer={layer} initial={it}
                                   onSave={(form) => update(it.id, form)}
                                   onCancel={() => setEditingId(null)} />
                      : <AssetRow key={it.id} item={it} grand={grandJpyMan || 1} ccy={ccy} fx={fx}
                                  onEditStart={() => setEditingId(it.id)}
                                  onDelete={() => remove(it.id, it.name)} />
                    )}
                    {addingLayer === layer
                      ? <AssetForm layer={layer} initial={null}
                                   onSave={create}
          ...
          <div className="assets-grand">
            ...
            <span className="assets-grand-amount">
              ...
              <span className="muted" style={{fontSize: 11}}>{CCY_DISP[ccy].suffix}</span>
            </span>
          </div>
        </div>
        ...
        <div className="assets-chart">
          ...
          {grandJpyMan > 0
            ...
            : <div className="muted" style={{padding: 60, textAlign: 'center'}}>No assets yet — add one on the left.</div>
          }
          <div className="assets-chart-legend">
            {groups.map(g => (
              <div key={g.key} className="assets-legend-row">
                <span className="assets-legend-dot" style={{background: g.color}}></span>
...
// Pin the Portfolio's footprint so the size toggle doesn't apply to it:
// always full-width (matches the panel-wide CSS class) and 1.5× the standard
// large height (span 3 rows vs the default span 2). EditPanel hides the
// size button when fixedSize is set; Panel reads rowSpan in core.jsx.
