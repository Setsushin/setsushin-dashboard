...
// Phase 2 will move SOURCES to KV so editing doesn't require deploy.
...
export function parseFeed(xml, src) {
  ...
  for (const m of xml.matchAll(/<entry\b[^>]*>([\s\S]*?)<\/entry>/g)) {
    ...
    items.push({
      ...
      published: extract(block, /<published>([^<]+)<\/published>/) || extract(block, /<updated>([^<]+)<\/updated>/) || '',
    });
  }
