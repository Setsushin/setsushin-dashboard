...
//
// GET  /api/assets  → [{id, layer, sublayer, name, jpy_man, usd, exposure, account, sort_order}, …]
// POST /api/assets  → body: {layer, name, jpy_man, exposure, usd?, sublayer?, account?, sort_order?}
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const { layer, name, jpy_man, usd, exposure, sublayer, account, sort_order } = r.data;

  const db = env.setsushin_dash;
  const { meta } = await db
    .prepare(
      `INSERT INTO assets (user_email, layer, sublayer, name, jpy_man, usd, exposure, account, sort_order)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(email, layer, sublayer ?? null, name, jpy_man, usd ?? null, exposure, account ?? null, sort_order)
    .run();
